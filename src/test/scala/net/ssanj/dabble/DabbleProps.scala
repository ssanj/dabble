...
trait DabbleProps {
 ...
 private[dabble] def genLibraryVersion: Gen[String] = for {
  ...
  private lazy val sbtTemplate1 = """name := "Dabble"
...
"""
  ...
  private[dabble] val sbtTemplate2 =
    s"//generated by Dabble ${DabbleInfo.version}${newline}"       +
    s"${templateName}${newline}"         +
    s"${templateOrganisation}${newline}" +
    s"${templateVersion}${newline}"      +
    s"${templateScalaVersion}"

  private[dabble] def genSbtTemplateContent: Gen[SbtTemplateContent] =
    Gen.oneOf(sbtTemplate1, sbtTemplate2).map(SbtTemplateContent)
